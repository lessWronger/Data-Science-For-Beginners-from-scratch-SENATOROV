...
#     Питон вызывает инструкции из языка С и код выполняется не напрямую в  машинных инструкциях, а через интерпретатор (CPython). Скорость как раз таки обуславливается ядром на языке С. Это позволяет программисту писать код быстро и просто, а «тяжёлая работа» уходит на оптимизированном коде C
#
...
#        4. В отличие от PyPy, CPython не использует JIT-компиляцию — поэтому он медленнее, но стабильнее и совместимее
